...
var check_detail_id =getParamFromWindowName("check_detail_id");
var date_type = "";//日期的类型
var end_date = "";//结束的日期
	...
	if(isVip=="true"){
		...
		if(result.isLogin){
			$(".midd_left_wrap").load(getwebroot()+"vip/usercenter/saleManage/vipSaleLeftMenu.html",function(){
				$("#evaluation").children().eq(5).children().eq(2).find("a").prepend(">>");
				$("#evaluation").children().eq(5).children().eq(2).addClass("currVip");
			});
		}
	}else{
 ...
 * 鼠标上移、下移效果
 */
$(".feedback_list_body").mouseover(function(){
	$(this).addClass("feedback_list_body_hover");
	$(this).find(".listTip").addClass("listTip_hover");
});
$(".feedback_list_body").mouseleave(function(){
	$(this).removeClass("feedback_list_body_hover");
	$(this).find(".listTip").removeClass("listTip_hover");
});
/**
* 取到所有参数的值
* orgizaParam
...
if(check_detail_id != ""){
	 params.check_detail_id = check_detail_id;
			}
