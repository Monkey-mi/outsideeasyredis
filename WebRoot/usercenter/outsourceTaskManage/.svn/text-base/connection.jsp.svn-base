...
<div class="midd_wrap p10 pb30 bg_white">
	...
	<textarea rows="2" cols="5" id="message_text_box" class="message_text_box" style="border:1px solid #d0d0d0; height:70px; width:98%; margin:8px 0px 8px 9px;display:inline-block;"></textarea>
	<!-- <div id="submit_message_button" class="submit_message_button">留 言</div> -->	
	<div class="comm_inner_line_wrap clearfix" id ="comm_img_wrap"></div>
	<div id="message_show_block" class="message_show_block">
	</div>
</div>
	...
	$(function(){
		...
		params.t_id=taskid;	
   		var fn=function(result){
   			...
   			state = task.state;
   			connectionButton(task.state);//是否改变按钮状态
...
function stateLoisgisticsimgShow(state){
		...
		var fn = function(result){
			if(result.ajaxErrorCode == 200){
				...
				for(var i = 0; i < communications.length; i++){//单个信息模块加载
					...
					var reviewButton ="";
					if(state ==10||state ==15||state ==20){
						reviewButton ="<span style='text-align:center' class='f_l'><img class='return_button' src='/newresources/images/return_back_button.png'/></span>";
					}
					var fileimgtext = "";
					...
					for(var j = 1; j < communications[i].length; j++){//单个信息模块回复信息加载
						...
						return_back_block += "<div class='message_return_single_block clearfix'>"
													+"<div class='company_logo' style=''><img src=" + srcPathij + "></div>"
					...
					if(communications[i][0].mogodb_id != null){
						...
						if(communications[i][0].fileCount>0){
							fileimgtext="<a id='td_imglist_title' class='td_imglist_title f_l ml10' href='javascript:void(0)' onClick='toggle_imglist(this,"+communications[i][0].id+")'>附件图片</a>"
						}else{
						fileimgtext = "";
						}
	...
	function submitAskMessage(){
		...
		params.taskId = taskid;
		params.companyId = company_id;
		params.moduleType = 0;//0：任务单；1：订单；
		params.file_type = 43;
		arr = [];//声明对象中的对象         
		...
		var fn = function(result){
			if(result.ajaxErrorCode == 200){
				...
				var return_back_block_single = "<div class='message_return_single_block clearfix'>"
													+"<div class='company_logo'><img src="+ srcPath +"></div>"
													+"<div class='return_message_info' style='margin-bottom:30px;'>"
	...
	function addlogisticImg(obj,returnType){
		 ...
		 var filename=$("#"+id1).val();//上传的文件信息	 
		   ...
		   if(filename!=""){
				...
				var params = {"t_id":taskid,"file_type":43,"companyId":company_id};
			    ...
			    addInputUtilFile(fileurl,params,id1,fn);
		   	   ...
		   	   /* $.ajaxFileUpload({
			        ...
			        data: {"t_id":taskid,"file_type":43,"companyId":company_id},  //任务id参数		  
