...
function getShippingDetails(){
	currentPage = 0;
	orgizaParam();
	var url = "taskDeliverGood/getTaskDeliverGoodsList.do";
	var params={};	
...
/**点击“我要发货”，进入发货登记页面
 * goStartDelivery void
 * @author wangjialin
 * 2016-11-21 下午1:55:09
 */
function goStartDelivery(){
	window.location.href=getwebroot()+"taskDeliverGood/deliveryRegister.htm";
}

/**查看发货单详情
 * queryInvoiceInfo void
 * @author wangjialin
 * 2016-11-21 下午3:30:49
 */
function queryInvoiceInfo(deliver_number){
	var param ={"deliver_number":deliver_number};
	addParamsToWindowName(param);
	window.location.href=getwebroot()+"taskDeliverGood/invoiceInfo.htm";	
}
/**运单详情
 * queryInvoiceInfo void
 * @author wangjialin
 * 2016-11-21 下午3:30:49
 */
function queryWaybillInfo(shipping_number){
	var param ={"shipping_number":shipping_number};
	addParamsToWindowName(param);
	window.location.href=getwebroot()+"taskDeliverGood/waybillInfo.htm";	
}