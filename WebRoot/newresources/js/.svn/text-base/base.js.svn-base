...
$(function(){
	...
	$("body").prepend("<script type='text/javascript' src='/newresources/js/spin/spin.js'></script>");
	$("body").prepend("<script type='text/javascript' src='/newresources/js/doT/doT.min.js'></script>");
	$("head").append("<script type='text/javascript' src='/newresources/js/login/plugLogin.js'></script>");
	if(userInfo!=null){
			...
			||nowUrl==getwebroot()+"vip/vipLogin.html"||nowUrl==getwebroot()+"helpCenter/newUser/registerAndLogin.html"||nowUrl.indexOf(getwebroot()+"userInfo/vipFindPwd.htm") != -1
...
/**
 * 上传所有的文件的js方法
 * fileurl:上传的文件的接口路经，param：上传的参数，fileElementId：上传的文件所在input框的id名；
 * fn：上传成功后的回调函数
* addComUtilImg
* @param obj
* @return void
