...
//update by Wangjialin:新增的全局变量,用于判断是直接输入的新名称(false)还是已有的选项(true);
var flag=true;
	/**
	 * Utility functions
	 */
	...
	$.extend(Plugin.prototype, {
		...
		_blur: function(){
			...
			if(flag){
				this._selectByIndex(index);
			}else{//update by Wangjialin :输入下拉列表中暂无的选项时，手动更改下拉框的选中值(PS:下拉框选项值不可为-1)
				this.$container.parent().find("select").val(-1);
				$("#phone_number").val("");//联动项内容清空
			}
		},
