...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        bind: function () {
            ...
            this.$dropdown.on('click', '.city-select a', function () {
                var $select = $(this).parents('.city-select');//地区分类
                var $active = $select.find('a.active');//选择具体的地区
                var last = $select.next().length === 0;//是否是最后一个地区分类
                $active.removeClass('active');
                $(this).addClass('active');
                //if ($active.data('code') !== $(this).data('code')) {//只要点击就会调到下一级
                    $select.data('item', {
        ...
        getText: function () {
            ...
            this.$dropdown.find('.city-select')
                .each(function () {
                    ...
                    if (item) {
                        text += ($(this).hasClass('province') ? '' : '<label>/</label>') + '<span class="select-item" data-count="' +
        ...
        getVal: function () {
            ...
            this.$dropdown.find('.city-select')
                .each(function () {
                    var item = $(this).data('item');
                    if (item) {
                        text += ($(this).hasClass('province') ? '' : '<label>/</label>') + item.address;
                    }
                });
            return text;
        },
    ...
    /*$(function () {
        $('[data-toggle="city-picker"]').citypicker({ province: '浙江省',
        city: '湖州市'});
    });*/
});